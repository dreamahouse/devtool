...
export function getDataList(src, locale) {
  const dataPath = path.join(cwd(), "data", "json", locale, "tools", src);
    ...
    if (category.name.toLowerCase() == keyword.toLowerCase()) {
      ...
      for (const item of dataList) {
        if (item.name.toLowerCase().includes(keyword.toLowerCase())) {
          ...
          item.tags &&
          item.tags.some((tag) => tag.toLowerCase() == keyword.toLowerCase())
        ) {
          // search by tags
...
export function getChangelog() {
  const dataPath = path.join(cwd(), "data", "json", "changelog.jsonc");
  const dataList = jsonc.parse(fs.readFileSync(dataPath, "utf8"));
  return dataList;
}
